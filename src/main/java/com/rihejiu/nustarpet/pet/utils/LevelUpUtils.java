...
public class LevelUpUtils {
    ...
    public static ItemStack petLevelUp(Player player, int exp, ItemStack pet) {
        // 异兽当前最大经验和当前经验
        int maxexp = Pet.getPetMaxExp(pet);
        int oldexp = Pet.getPetExp(pet);
    ...
    public static ItemStack recursionLevelUp(int exp, int maxexp, ItemStack pet,int oldexp){
        ...
        if (exp > maxexp - oldexp){
            ...
            pet = NBTUtils.setStringTag(pet,"MaxExp",String.valueOf(Pet.getPetNextLevelMaxExp(pet)));
            // 如果现在喂养经验大于异兽最大经验值执行 递归 反之设置异兽当前经验值
            int newmaxexp = Pet.getPetMaxExp(pet);
            if (exp > newmaxexp) {
